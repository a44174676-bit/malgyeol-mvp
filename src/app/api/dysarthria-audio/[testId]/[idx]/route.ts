...
import { parseBatteryResults } from "@/lib/battery";
...
export async function GET(
  ...
  const test = await db.dysarthriaTest.findUnique({ where: { id: testId } });
  if (!test) return new NextResponse("검사를 찾을 수 없습니다.", { status: 404 });

  const audioPath = parseBatteryResults(test.resultsJson)[Number(idx)]?.audioPath;
